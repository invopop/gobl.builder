...
<div class="absolute flex py-2 w-full h-full top-0 left-0 z-0 rounded {clasess}" transition:fade={{ duration: 200 }}>
  <div class="flex-shrink-0 absolute right-full top-0 bg-gray-50 rounded">
    <div class="flex align-middle text-gray-100">
      {#if showAdd}
        <Tooltip label="Add field">
          <button on:click={handleClickAddButton} class="hover:bg-gray-100 p-2.5">
            <svg viewBox="0 0 16 16" class="h-4 w-4 p-0.5 plus">
              <path
      ...
      {#if showOptions}
        <Tooltip label="Options">
          <button on:click={handleClickOptionsButton} class="hover:bg-gray-100 py-2.5 px-1">
            <svg viewBox="0 0 10 10" class="h-4 w-4 p-0.5 dragHandle">
              <path
