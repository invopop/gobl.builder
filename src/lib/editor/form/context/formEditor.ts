...
import { fieldIsArray, fieldIsObject, getJSONFromUIModel, getUIModel, type SchemaValue } from "$lib/editor/form/utils/schema.js";
...
export function createFormEditorContext(jsonSchemaURL: Readable<string>): FormEditorContextType {
  const uiModel = derived<Readable<any>[], UIModel | undefined>([jsonSchemaURL, editorJSON], ([$schema, $value], set) => {
    (async () => {
