...
export async function getUIModel<V extends SchemaValue>({
  ...
  switch (schema.type) {
    case "object": {
      ...
      out.options = Object.entries(schema.properties || {})
...
export function getJSONFromUIModel(field: UIModelField): any {
  switch (field.type) {
    case "object": {
      return Object.entries(field.children || {}).reduce((acc, [k, v]) => {
        acc[k] = getJSONFromUIModel(v);
        return acc;
      }, {} as Record<string, any>);
    }
    case "array": {
      return Object.values(field.children || {}).map((v) => getJSONFromUIModel(v));
    }
    default: {
      return field.value;
    }
