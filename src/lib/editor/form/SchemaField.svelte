<script lang="ts">
  ...
  import type { SchemaFieldProps } from '$lib/types/editor'

  const { jsonSchema, updateSchema } = getBuilderContext()

  let { isEmptySchema = true }: SchemaFieldProps = $props()
...
{/if}
{#if isEmptySchema}
  <SelectSchemas placeholder="Select a $schema..." onChange={update} />
{:else}
  <div class="flex items-center justify-center">
    <button
      class="border py-2 px-6 hover:border-blue-400"
      onclick={() => updateSchema($jsonSchema || '')}>Fix</button
    >
  </div>
{/if}
